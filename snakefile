...
rule LD_pruning:
	...
	--keep 1kg_IDS/{wildcards.POP}.txt \
        --make-bed \
...
rule calc_gamma:
    input:
        ...
        "output/pruned/{POP}/{POP}_{MAF}.{window}_{step}_{thresh}.DA.prune.in"
    output:
        "output/gamma/{POP}/{POP}_{MAF}.{window}_{step}_{thresh}_gamma.txt"
    script:
        "scripts/calc_gamma.R"

rule plot_gamma:
    input:
        "output/gamma/{POP}/{POP}_{MAF}.{window}_{step}_{thresh}_gamma.txt"
    output:
        "output/figures/{POP}/{POP}_{MAF}.{window}_{step}_{thresh}_gamma.png"
    script:
        "scripts/plot_gamma.R"